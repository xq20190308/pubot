...
def requests_eventJoin(actiId):
    eventJoinResponse = requests.get(parseConfig.url_eventDetail() + f"&oauth_token={parseConfig.token_oauth_token()}&oauth_token_secret={parseConfig.token_oauth_token_secret()}&actiId={actiId}")
    if not eventJoinResponse.ok:
        return "报名失败"
    return "报名成功"